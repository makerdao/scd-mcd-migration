...
contract ProxyLib is DSMath {
    function migrate(address payable scdMcdMigration, bytes32 cup) public returns (bytes12 cdp) {
        ...
        if (ok && uint(val) != 0) {
            tub.gov().move(msg.sender, address(scdMcdMigration), wdiv(tub.rap(cup), uint(val)));
        }
